...
An `openenv.toml` file is built around five primary sections:
...
- `openenv.toml` keeps non-secret runtime configuration
...
next to the manifest and stores only file references in `openenv.toml`.
...
- `openenv.toml` holds the declarative structure
