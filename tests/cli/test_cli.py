...
class CliTests(unittest.TestCase):
    def test_init_creates_manifest(self) -> None:
        with workspace_dir() as temp_dir:
            manifest_path = temp_dir / "openenv.toml"
    ...
    def test_build_uses_default_tag(self) -> None:
        with workspace_dir() as temp_dir:
            ...
            self.assertEqual(tag, "openenv/ops-agent:1.2.3")
            self.assertEqual(
                build_image.call_args.kwargs["build_args"],
                {
                    "OPENENV_SKILL_SCAN_FAIL_ON_SEVERITY": "high",
                    "OPENENV_SKILL_SCAN_FORMAT": "summary",
                    "OPENENV_SKILL_SCAN_POLICY": "balanced",
                },
            )
            self.assertTrue(dockerfile_path.exists())
            self.assertTrue(compose_path.exists())
    ...
    def test_build_passes_custom_skill_scan_settings(self) -> None:
        with workspace_dir() as temp_dir:
            ...
            self.assertEqual(
                build_image.call_args.kwargs["build_args"],
                {
                    "OPENENV_SKILL_SCAN_FAIL_ON_SEVERITY": "medium",
                    "OPENENV_SKILL_SCAN_FORMAT": "json",
                    "OPENENV_SKILL_SCAN_POLICY": "strict",
                },
            )
...
""".strip(),
            ...
            self.assertIn("OPENCLAW_IMAGE=openenv/sidecar-agent:0.1.0", env_text)
    ...
    def test_validate_prints_summary(self) -> None:
        ...
        with redirect_stdout(stdout):
            exit_code = main(
                ["validate", "--path", str(FIXTURES / "example.openenv.toml")]
            )
